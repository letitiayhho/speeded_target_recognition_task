...
% If the input is a cellstr, it will be treated as in-use RTBox ports. Then this
% function will open 1st available RTBox port, and return the port name as 1st
% output, and the 2nd output will be a struct containing serial handle, RTBox
% version, clock unit, latency timer and host MAC address. In case no available
% RTBox is found, the port will be empty, and the 2nd struct output will show
% available and busy ports.

% 090901 Wrote it (XL)
% 100102 use non-block write to avoid problem for some ports
% 120101 add second output
% 120701 Return from bootloader or RTBboxADC if needed
...
if ~toOpen && logical(allPorts) % ask all ports, so close open ones
...
% 3-digit which seems from location ID dropping the trailing zeros. Some say
...
%  D in port does not match the starting hex in location ID, and does not
...
for i = regexp(str, '"idProduct"\s*=\s*0x6001')
    ...
    if ~isempty(iv), IDs{end+1} = regexp(str(i1:i2), expr, 'match', 'once'); end
end
...
elseif ismac
    ...
    errmsg = ['Fail to get LatencyTimer. Please start Matlab/Octave as ' ...
        'sudo and run PsychLinuxConfiguration'];
    if nargout<2, warning(warnID, WrapString(errmsg)); end
end
%%
