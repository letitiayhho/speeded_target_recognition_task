function write_output(subject, block, trial, vowel, stim_start, stim_end, pressed, rt, resp, correct)
    ...
    if ~(exist(fpath, 'file') == 2) % ~isfile(fpath) in later versions
        cols = 'subject,block,trial,vowel,stim_start,stim_end,pressed,rt,resp,correct';
        row = [cols row];
        f = fopen(fpath, 'wt'); 
        fprintf(f, row);
        fclose(f);
